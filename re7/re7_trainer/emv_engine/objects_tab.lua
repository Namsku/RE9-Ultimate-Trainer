...
local function scan_enemies()
    ...
    -- RE7: Find enemies via EnemyOrder components
    ...
    for i = 0, math.min(n - 1, 200) do
        pcall(function()
            ...
            -- Try to read HP
            pcall(function()
                local status = eo:get_field("<EnemyStatus>k__BackingField") or eo:call("get_EnemyStatus")
                if status then
                    local hp = status:get_field("Health")
                    local maxhp = status:get_field("MaxHealth")
    ...
    -- Also try PlayerOrder for player
    pcall(function()
        local ptd = sdk.find_type_definition("app.PlayerOrder")
        if not ptd then return end
        local pcomps = scene:call("findComponents(System.Type)", ptd:get_runtime_type())
        if not pcomps then return end
        local pn = pcomps:call("get_Count") or 0
        for i = 0, math.min(pn - 1, 1) do
            pcall(function()
                local po = pcomps:call("get_Item", i)
                if not po then return end
                local go = po:call("get_GameObject")
                if not go then return end
                local entry = make_entry(go)
                if entry then
...
local function scan_items()
    ...
    -- RE7: Try multiple item-related component types
    local item_types = {
...
local function scan_gimmicks()
    ...
    -- RE7: Try InteractGimmickController or GimmickBase
    local gimmick_types = {
        "app.InteractGimmickController",
        "app.GimmickSetParts",
        "app.GimmickBase",
    }
    ...
    for _, type_name in ipairs(gimmick_types) do
        local td = sdk.find_type_definition(type_name)
        if td then
            pcall(function()
                local comps = scene:call("findComponents(System.Type)", td:get_runtime_type())
    ...
    return out, total, total .. " gimmicks"
end
