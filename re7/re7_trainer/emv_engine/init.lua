...
if not dir then dir = "reframework/autorun/re7_trainer/emv_engine/" end
...
-- Phase 1: Foundation
local RE9_OFFSETS        = load_module("re7_offsets")
...
if not RE9_OFFSETS then
    -- Offsets not found — create a dummy so EMV doesn't abort
    RE9_OFFSETS = { offsets = {}, get = function() return 0x0 end, dump = function() end }
    if log then log.warn("[EMV] Offsets module not found — using defaults") end
end
if not SafeMemory then
    if log then log.error("[EMV] CRITICAL: SafeMemory failed. EMV disabled.") end
    return
end
